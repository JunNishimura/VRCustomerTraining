...
public class AutoGeneratedScenarioController : MonoBehaviour
{
    ...
    public GameObject[] _entranceAvatars; // エントランスのアバター
    ...
    Animator[] _entranceAvatarAnimators = new Animator[3]; // 入店客のanimator
    ...
    bool _isFBboardDisplayed = false; // フィードバックボード表示したか否かのフラグ
    ...
    /* 
    1. FlagManagerについて
        Flags[0] ～ flags[5]はシナリオ番号
	    Flags[6] ～ flags[8]は声掛け番号
        Flags[9]は全シナリオ完了番号
    */
    ...
    void Start()
    {
        ...
        for (int i = 0; i < _entranceAvatars.Length; i++)
        {
            _entranceAvatarAnimators[i] = _entranceAvatars[i].GetComponent<Animator>();
        }
        ...
        // チェックポイントシナリオの初期化
        _checkPointScenarioList = new List<int>();
    ...
    void Update()
    {
            ...
            if (scenario["task"] == "片付け")
            {
                if (_elapsedTime > activateTime && isScenarioActivated)
                {
                    ...
                    if (_cleanPlageJudge.IsPlatePicked(plateName) && !FlagManager.Instance.flags[scenarioNum])
                    {
                        ...
                        {
                            FlagManager.Instance.flags[6 + _checkPointCount] = true;
                        }
                        // チェックポイントシナリオの追加
                        _checkPointScenarioList.Add(scenarioNum);
                    }
            ...
            else if (scenario["task"] == "入店")
            {
                if (_elapsedTime > activateTime && isScenarioActivated)
                {
                    if (_entranceEnterCube.GetEnter() == 1)
                    {
                        ...
                        // 視線の確認
                        if (IsPlayerLookAtCustomer(_entranceAvatars[_entranceAvatarCounter]))
                        {
                            // 音声認識完了のチェック
                            if (Speech_Recognition_Manager.rec_complete)
                            {
                ...
                {
                    // 入店客の表示
                    _entranceAvatars[_entranceAvatarCounter].SetActive(true);
                    _navMeshAgent = _entranceAvatars[_entranceAvatarCounter].GetComponent<NavMeshAgent>();
        ...
        if (_entranceScenarioNum >= 0 && FlagManager.Instance.flags[_entranceScenarioNum])
        {
            ...
            // prevEntranceAvatarCounterを使用する理由は、アバターが移動する時点で既に、_entranceAvatarCounterが更新されて+1されているから
            int prevEntranceAvatarCounter = _entranceAvatarCounter - 1;
            _entranceAvatarAnimators[prevEntranceAvatarCounter].SetBool("walk", true);
            _entranceAvatars[prevEntranceAvatarCounter].GetComponent<LookAtController>().enabled = false;

            if (Vector3.Distance(_chairs[tableNum].transform.position, _entranceAvatars[prevEntranceAvatarCounter].transform.position) < 1)
            {
                if (tableNum > 1) // table 2, 3, 4, 5の場合
                {
                    _entranceAvatars[prevEntranceAvatarCounter].transform.rotation = Quaternion.Slerp(_entranceAvatars[prevEntranceAvatarCounter].transform.rotation, Quaternion.Euler(0f, 0f, 0f), 0.2f);
                }
                else // table 0, 1の場合
                {
                    _entranceAvatars[prevEntranceAvatarCounter].transform.rotation = Quaternion.Slerp(_entranceAvatars[prevEntranceAvatarCounter].transform.rotation, Quaternion.Euler(0f, 90f, 0f), 0.2f);
                }
            }

            if (Vector3.Distance(_chairs[tableNum].transform.position, _entranceAvatars[prevEntranceAvatarCounter].transform.position) < 0.71)
            {
                _entranceAvatarAnimators[prevEntranceAvatarCounter].SetBool("sitting", true);

                // 入店が複数の場合を想定は_entranceScenarioNumを更新
                for (int scenarioNum = 0; scenarioNum < _generatedScenarioList.Count; scenarioNum++)
                {
                    if (_generatedScenarioList[scenarioNum]["task"] == "入店" && !FlagManager.Instance.flags[scenarioNum])
                    {
                        _entranceScenarioNum = scenarioNum;
                        break;
                    }
                }
            }
        ...
        // 全シナリオが完了した際の処理
        if (FlagManager.Instance.flags[9] && !_isFBboardDisplayed)
        {
            // ガイドパネルの更新
            ...
            for (int i = 0; i < _taskFinishedOrderList.Count; i++)
            {
                ...
                UIManager.Instance.result.text += (i + 1) + "番目：" + tableNum + "番テーブル " + task + "    ";
                if (i % 2 == 1)
                {
                    UIManager.Instance.result.text += "\n";
                }
            ...
            // 声掛けの有無を表示
            bool isKoegakeComplete = FlagManager.Instance.flags[6] && FlagManager.Instance.flags[7] && FlagManager.Instance.flags[8];
            if (isKoegakeComplete)
            {
            ...
            else
            {
                ...
                {
                    // 声掛けフラグは6,7,8
                    if (!FlagManager.Instance.flags[6+i])
                    {
                        string task = _generatedScenarioList[_checkPointScenarioList[i]]["task"];
                        int tableNum = int.Parse(_generatedScenarioList[_checkPointScenarioList[i]]["table"]) + 1;
                        if (task == "注文")
                        {
                            UIManager.Instance.evaluation.text += tableNum + "番テーブルへの声掛けを忘れています。\n";
                        }
                        else if (task == "片付け")
                        {
                            UIManager.Instance.evaluation.text += tableNum + "番テーブルの片付けは後で行いましょう。\n";
                        }
                    }
                }
                UIManager.Instance.evaluation.text += "お客さんを順番待ちさせてしまう場合には、お待たせすることを伝えましょう。\n";
            }

            // フィードバックボード表示フラグを立てる
            _isFBboardDisplayed = true;
        }
    }
    ...
    void ScenarioGenerate()
    {
        ...
        for (int i = 0; i < _scenarioCount; i++)
        {
            ...
            while (true)
            {
                ...
                if (task == "入店" && !_isEntranceGenerated) // 1つ目の入店タスクは_entranceScenarioNumを代入
                {
                    if (i % 2 == 1)
                    {
            ...
            else if (_scenarioCount == 4 || _scenarioCount == 5)
            {
                FlagManager.Instance.flags[8] = true;
            }
        }
    ...
    void ScenarioCompletedJudge()
    {
        ...
        if (!FlagManager.Instance.flags[9])
        {
            for (int i = 0; i < _generatedScenarioList.Count; i++)
            {
